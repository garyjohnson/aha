...
class SQL extends mysqli {
	...
	private $debug = false;
	/** Determines if there is an SQL connection or not **/
	...
	/**
	 * Constructs a new MySQLi Object. Connection information can be input as individual arguments
	 ...
	 * @throws SQLException
	 */
	public function __construct($host = "localhost", $user = "root", $pass = "pass", $db = "", $prefix = "", $debug = FALSE) {
		...
		if(mysqli_connect_errno() == 1049)
			...
			throw new SQLException("Failed to connect to the MySQL server. (".mysqli_connect_errno().") ".mysqli_connect_error(), SQLException::MYSQL_CONNECT);

		// Set prefix and debugging status
	 ...
	 * @return The <code>MySQLi Result Set</code> of results or <code>TRUE</code> on success. Otherwise
	 * <code>FALSE</code>.
	 * @throws SQLException
	 */
	public function query($query, $debug = FALSE) {
		...
		// Do the query
		$s = microtime(true);
		$this->lresult = @parent::query($query);
		$e = microtime(true);
		$time = $e-$s;
		
		// Check debugging options
		if($debug === "now")
			echo "SQL_DEBUG: \"{$query}\"<br />Error:{$this->error()}";
	 ...
	 * @param MySQLi_result_set $result - The result set to fetch from. (<code>$this->lresult</code>)
	 * @return Array of values for the row or FALSE if failed to fetch row.
	 */
	public function fetch($type = MYSQLI_BOTH, $result = NULL) {
		if($result === NULL)
			$result = $this->lresult;
		
		if(!is_null($result) && $result instanceof mysqli_result)
	 ...
	 * @return Array of values for the row or FALSE if failed to fetch row.
	 */
	public function fetchAll($type = MYSQLI_BOTH, $result = NULL) {
		if($result === NULL)
			$result = $this->lresult;
		
		if(!is_null($result) && $result instanceof mysqli_result) {
			if(method_exists($result, "fetch_all"))
				$row = @$result->fetch_all($type);
			else {
				$rows = Array();
	...
	public function rows() {
		...
		else if($this->lresult !== FALSE && $this->lresult instanceof mysqli_result)
			return @$this->lresult->num_rows;
		else
			return -1;
	}
...
<tr><th>Rows</th><th>Time</th><th>Query</th></tr>";
		...
		foreach($this->qInfo as $info) {
			...
			echo "<tr style='background-color: #{$color};'><td>{$info["rows"]}</td><td>{$info["time"]}</td><td>{$info["query"]}</td></tr>\n";
